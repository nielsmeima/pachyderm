...
`,
	...
	inspectJob := &cobra.Command{
		...
		Run: cmdutil.RunFixedArgs(1, func(args []string) error {
			client, err := pachdclient.NewOnUserMachine(!*noMetrics, "user")
			if err != nil {
				return err
			}
			defer client.Close()
...
` + codeend,
		Run: cmdutil.Run(func(args []string) error {
			...
			c, err := pachdclient.NewOnUserMachine(!*noMetrics, "user")
			if err != nil {
				return err
			}
			defer c.Close()
...
`,
	...
	createPipeline := &cobra.Command{
		...
		Run: cmdutil.RunFixedArgs(0, func(args []string) (retErr error) {
			return pipelineHelper(!*noMetrics, false, build, pushImages, registry, username, pipelinePath, false)
		}),
	}
	...
	updatePipeline := &cobra.Command{
		...
		Run: cmdutil.RunFixedArgs(0, func(args []string) (retErr error) {
			return pipelineHelper(!*noMetrics, reprocess, build, pushImages, registry, username, pipelinePath, true)
		}),
	}
