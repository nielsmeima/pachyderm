...
`,
	...
	createRepo := &cobra.Command{
		...
		Run: cmdutil.RunFixedArgs(1, func(args []string) error {
			c, err := client.NewOnUserMachine(!*noMetrics, "user")
			if err != nil {
				return err
			}
			defer c.Close()
...
` + codeend,
		Run: cmdutil.RunBoundedArgs(1, 2, func(args []string) error {
			cli, err := client.NewOnUserMachine(!*noMetrics, "user")
			if err != nil {
				return err
			}
			defer cli.Close()
...
` + codeend,
		Run: cmdutil.RunBoundedArgs(2, 3, func(args []string) (retErr error) {
			c, err := client.NewOnUserMachine(!*noMetrics, "user", client.WithMaxConcurrentStreams(parallelism))
			if err != nil {
				return err
			}
			defer c.Close()
...
` + codeend,
	...
	mount := &cobra.Command{
		...
		Run: cmdutil.RunFixedArgs(1, func(args []string) error {
			client, err := client.NewOnUserMachine(!*noMetrics, "fuse")
			if err != nil {
				return err
			}
			defer client.Close()
